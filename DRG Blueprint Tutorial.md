...
![0-1](./DRG Blueprint Tutorial.assets/0-1.png)

另外，在选项中建议把除了核心组件以外的内容取消勾选，我们不需要这些，并且能大幅减少存储空间的占用。

![0-2](./DRG Blueprint Tutorial.assets/0-2.png)
...
版本最好选择2019或2022，更高的版本可能会有兼容性问题。鉴于旧版的 Visual Studio Build Tools 下载地址不太好找，这里也提供了链接。 [单击此处下载](https://aka.ms/vs/17/release/vs_buildtools.exe)

我们需要在工作负荷中勾选使用 C++ 的桌面开发，同时确保右侧详细信息仅勾选 MSVC 和 Windows SDK（你是win11就勾选11的版本）。

![0-3](./DRG Blueprint Tutorial.assets/0-3.png)

然后在单个组件中，至少勾选一个版本在4.6.1以上的 .NET Framework SDK，否则虚幻打包时会报错。

![0-4](./DRG Blueprint Tutorial.assets/0-4.png)
...
![0-5](./DRG Blueprint Tutorial.assets/0-5.png)
...
![0-6](./DRG Blueprint Tutorial.assets/0-6.png)

取消勾选本地化节点命名。（建议）

![0-7](./DRG Blueprint Tutorial.assets/0-7.png)

然后打开项目设置，选择打包。

![0-8](./DRG Blueprint Tutorial.assets/0-8.png)

将使用 pak 文件取消勾选。

![0-9](./DRG Blueprint Tutorial.assets/0-9.png)

同时确保编译设置为发行。

![0-10](./DRG Blueprint Tutorial.assets/0-10.png)
...
![1-1](./DRG Blueprint Tutorial.assets/1-1.png)

右键空白区域新建蓝图类，选择 Actor。

![1-2](./DRG Blueprint Tutorial.assets/1-2.png)
